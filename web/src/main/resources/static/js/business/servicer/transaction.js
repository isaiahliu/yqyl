layoutApp.controller('contentController', function($scope, $http, $window, $filter, errorHandler) {
	...
	$scope.searchTransactions = function() {
		...
		if ($scope.filterData.fromDate != undefined && $scope.filterData.fromDate != null) {
			ajaxUrl += "&paymentFromDate=" + $filter('date')($scope.filterData.fromDate, 'yyyyMMdd');
		}

		if ($scope.filterData.toDate != undefined && $scope.filterData.toDate != null) {
			ajaxUrl += "&paymentToDate=" + $filter('date')($scope.filterData.toDate, 'yyyyMMdd');
		}

		if ($scope.filterData.staffNo != undefined && $scope.filterData.staffNo != "") {
		...
		if ($scope.filterData.paymentCode != undefined && $scope.filterData.paymentCode != "") {
			ajaxUrl += "&paymentCode=" + $scope.filterData.paymentCode;
		}
