...
layoutApp
		.controller(
				'contentController',
				function($scope, $http, $window, errorHandler, staffId) {
					$scope.dateOptions = {
						dateFormat : 'yy/mm/dd',
						changeMonth : true,
						changeYear : true,
					...
					if (staffId > 0) {
						...
						$http(
								{
									method : "GET",
									url : "/ajax/service/supplier/staff?rsexp=serviceCategories&searchAllStatus=true&id="
											+ staffId
								})
								.success(
										function(response) {
											...
											if ($scope.staff.photo != undefined
													&& $scope.staff.photo != null) {
												$scope.imageUrl = "/ajax/content/image/"
														+ $scope.staff.photo;
											}
											$http(
													{
						...
						if (staffId > 0) {
							...
							$http({
								...
								data : {
									...
									.success(
											function(response) {
												$window.location.href = "/servicer/staff/edit/"
														+ response.data[0].id;
											}).error(function(response) {
										errorHandler($scope, response);
									});
