		...
		.controller(
				...
				function($scope, $http, $window, errorHandler, serviceOrderId) {
					...
					$http(
							{
								method : "GET",
								url : "/ajax/user/order?uid="
										+ serviceOrderId
										+ "&rsexp=serviceInfo[serviceCategory,serviceSupplierClient]"
							})
							.success(
									function(response) {
										$scope.order = response.data[0];
										$scope.order.serviceDate = new Date(
												$scope.order.serviceDate);

										if ($scope.order.status.code == 'G') {
											$http(
													{
														method : "GET",
														url : "/ajax/service/supplier/"
																+ $scope.order.serviceInfo.serviceSupplierClient.id
