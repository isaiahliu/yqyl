				...
				function($scope, $http, $window, errorHandler) {
					...
					$scope.assignPrepare = function(order) {
						$http(
								{
									method : "GET",
									url : "/ajax/service/supplier/staff/available?serviceCategoryId="
											+ order.serviceInfo.serviceCategory.id
								}).success(function(response) {
					...
					$scope.assign = function(order) {
						...
						if (order.selectedStaff == undefined
								|| order.selectedStaff == "") {
							staff = order.availableStaffs[Math.floor(Math
									.random()
									* order.availableStaffs.length)];
						} else {
					...
					$scope.edit = function(order) {
						$window.location.href = "/servicer/order/edit/"
								+ order.id;
					};
