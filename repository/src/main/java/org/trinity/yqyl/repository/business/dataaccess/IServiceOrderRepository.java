		...
		final Specification<ServiceOrder> specification = (root, query, cb) -> {
			...
			if (searchingDto.getAppraiseLevel() != null) {
				switch (searchingDto.getAppraiseLevel()) {
					case 1:
						predicates.add(cb.greaterThanOrEqualTo(root.join(ServiceOrder_.appraise).get(ServiceOrderAppraise_.totalRate), 17));
						break;
					case 2:
						predicates.add(cb.between(root.join(ServiceOrder_.appraise).get(ServiceOrderAppraise_.totalRate), 12, 16));
						break;
					case 3:
						predicates.add(cb.lessThan(root.join(ServiceOrder_.appraise).get(ServiceOrderAppraise_.totalRate), 12));
						break;
					default:
						break;
				}
			...
			if (!StringUtils.isEmpty(searchingDto.getPaymentFromDate())) {
				...
				try {
					...
					predicates.add(cb.greaterThanOrEqualTo(root.join(ServiceOrder_.paymentTransaction).get(AccountTransaction_.timestamp),
							fromDate));
				} catch (final ParseException e) {
				}
			}
			...
			if (!StringUtils.isEmpty(searchingDto.getPaymentToDate())) {
				...
				try {
					...
					predicates.add(cb.lessThan(root.join(ServiceOrder_.paymentTransaction).get(AccountTransaction_.timestamp),
							calendar.getTime()));
				} catch (final ParseException e) {
				}
			}
