    ...
    default Page<ServiceInfo> query(final ServiceInfoSearchingDto searchingDto, final Pageable pagable) {
        final Specification<ServiceInfo> specification = (root, query, cb) -> {
            ...
            if (!StringUtils.isEmpty(searchingDto.getName())) {
                predicates.add(cb.like(root.get(ServiceInfo_.name), "%" + searchingDto.getName() + "%"));
            }
            ...
            case "sales":
                ...
                if ("desc".equals(searchingDto.getCustomSortedDirection())) {
                    query.orderBy(cb.desc(root.join(ServiceInfo_.serviceInfoStastic, JoinType.LEFT).get(ServiceInfoStastic_.orderCount)));
                } else {
                    query.orderBy(cb.asc(root.join(ServiceInfo_.serviceInfoStastic, JoinType.LEFT).get(ServiceInfoStastic_.orderCount)));
                }
                break;
            case "appraise":
                query.distinct(true);
                if ("desc".equals(searchingDto.getCustomSortedDirection())) {
                    query.orderBy(cb.desc(root.join(ServiceInfo_.serviceInfoStastic, JoinType.LEFT).get(ServiceInfoStastic_.appraiseAvg)));
                } else {
                    query.orderBy(cb.asc(root.join(ServiceInfo_.serviceInfoStastic, JoinType.LEFT).get(ServiceInfoStastic_.appraiseAvg)));
                }
                break;
            default:
                break;
